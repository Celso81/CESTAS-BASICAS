...
Antes do lançamento: preencher os dados comerciais e o catálogo real; liberar `check:release`; ajustar `mode` para `production`; publicar; verificar HTTPS, www, redirects e resposta 404; inspecionar o domínio publicado no Google Search Console e enviar `https://cestaspopulares.com.br/sitemap.xml`. Ativar noindex na prévia não produz ranqueamento; a configuração muda somente na versão comercial validada.
