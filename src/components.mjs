...
export function inquiry(config, catalog, topic = '') {
  ...
  <form class="inquiry-form" data-whatsapp="${e(config.whatsapp)}" data-brand="${e(config.brand)}" data-topic="${e(topic)}" hidden>
    <div class="form-row"><div><label for="city">Cidade <span>(opcional)</span></label><input id="city" name="city" list="inquiry-cities" maxlength="80" autocomplete="address-level2" placeholder="Sua cidade" aria-describedby="city-hint"><datalist id="inquiry-cities">${cities.map(city => `<option value="${e(city)}"></option>`).join('')}</datalist></div><div><label for="neighborhood">Bairro <span>(opcional)</span></label><input id="neighborhood" name="neighborhood" maxlength="80" autocomplete="off" placeholder="Seu bairro"></div></div>
  ...
  </form><div class="direct-contact"><p>Prefere ir direto ao atendimento?</p>${contactButton(config, 'Abrir WhatsApp', { topic }, 'button-outline')}<span>${e(formatWhatsapp(config.whatsapp))}</span></div></div>`;
}
