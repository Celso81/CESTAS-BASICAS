...
const links = [['/', 'Início'], ['/cestas-basicas/', 'Nossas cestas'], ['/como-comprar/', 'Como comprar'], ['/entregas/', 'Entregas'], ['/cidades/', 'Cidades'], ['/guias/', 'Guias'], ['/contato/', 'Contato']];
...
export function plannedCities(config, compact = false) {
  return `<div class="planned-city-block${compact ? ' compact-cities' : ''}"><h3>Recife e todas as cidades da Região Metropolitana</h3><p>Consulte a possibilidade de entrega na sua cidade. O atendimento, o frete e o prazo dependem de confirmação por bairro ou CEP.</p><ul class="all-cities">${config.delivery.plannedCities.map(city => { const entry = cities.find(item => item.name === city); return `<li>${entry ? `<a href="${cityPath(entry.slug)}">${e(city)}</a>` : e(city)}</li>`; }).join('')}</ul>${config.delivery.additionalPlannedCities?.length ? `<p class="additional-cities">Também sob consulta: <strong>${config.delivery.additionalPlannedCities.map(city => { const entry = cities.find(item => item.name === city); return entry ? `<a href="${cityPath(entry.slug)}">${e(city)}</a>` : e(city); }).join(', ')}</strong>.</p>` : ''}</div>`;
}
export function catalogCards(config, catalog, limit) {
  const products = confirmedProducts(catalog).slice(0, limit);
  if (!products.length) return `<div class="catalog-empty"><span class="empty-icon">${icon('basket')}</span><div><h3>Catálogo em confirmação</h3><p>Ainda não há cestas com composição e preço publicados. Consulte o atendimento para receber a lista dos alimentos, as quantidades e um orçamento para sua região.</p></div>${contactButton(config, 'Pedir composição e orçamento')}</div>`;
...
export function inquiry(config, catalog, topic = '', { city: initialCity = '' } = {}) {
  ...
  const products = confirmedProducts(catalog).filter(item => item.availability !== 'unavailable');
  const cities = [...new Set([...config.delivery.confirmedCities, ...config.delivery.plannedCities, ...config.delivery.additionalPlannedCities])];
  return `<div class="inquiry-panel"><span class="eyebrow">PEÇA OS DETALHES</span><h2>Prepare sua consulta.</h2><p>Informe sua região e a quantidade desejada para pedir um orçamento completo. Todos os campos são opcionais.</p>
  <form class="inquiry-form" data-whatsapp="${e(config.whatsapp)}" data-brand="${e(config.brand)}" data-topic="${e(topic)}" hidden>
    <div class="form-row"><div><label for="city">Cidade <span>(opcional)</span></label><input id="city" name="city" list="inquiry-cities" maxlength="80" autocomplete="address-level2" placeholder="Sua cidade" value="${e(initialCity)}" aria-describedby="city-hint"><datalist id="inquiry-cities">${cities.map(city => `<option value="${e(city)}"></option>`).join('')}</datalist></div><div><label for="neighborhood">Bairro <span>(opcional)</span></label><input id="neighborhood" name="neighborhood" maxlength="80" autocomplete="off" placeholder="Seu bairro"></div></div>
    <p class="field-hint" id="city-hint">Pode escrever outra cidade. As sugestões não confirmam cobertura; consulte a entrega no seu endereço.</p>
    <label for="quantity">Quantidade de cestas <span>(opcional)</span></label><input id="quantity" name="quantity" type="number" min="1" max="999" step="1" inputmode="numeric" placeholder="Quantas cestas deseja consultar?">
    ${products.length ? `<label for="basket">Cesta de interesse <span>(opcional)</span></label><select id="basket" name="basket"><option value="">Quero conhecer as opções</option>${products.map(item => `<option value="${e(item.name)}">${e(item.name)}</option>`).join('')}</select>` : ''}
    <details class="message-details"><summary>Conferir a mensagem antes de abrir o WhatsApp</summary><p class="message-preview"></p></details>
    <p class="form-privacy">Estes campos apenas preparam sua mensagem. Ela será aberta no WhatsApp para você conferir e enviar. <a href="/privacidade/">Saiba sobre privacidade</a>.</p>
    <button class="button button-primary" type="submit">${icon('chat')} Continuar no WhatsApp ${icon('arrow', 'button-arrow')}</button><p class="form-feedback" aria-live="polite"></p>
  </form><div class="direct-contact"><p>Prefere ir direto ao atendimento?</p>${contactButton(config, 'Abrir WhatsApp', { topic, city: initialCity }, 'button-outline')}<span>${e(formatWhatsapp(config.whatsapp))}</span></div></div>`;
}
export function layout(config, page, body, mode, assets) {
